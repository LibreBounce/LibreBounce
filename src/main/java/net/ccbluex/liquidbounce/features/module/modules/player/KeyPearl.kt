...
object KeyPearl : Module("KeyPearl", Category.PLAYER, subjective = true, gameDetecting = false) {
    ...
    private fun throwEnderPearl() {
        ...
        if (!delayedSlotSwitch || SilentHotbar.currentSlot == pearlInHotbar) {
            ...
            sendPacket(PlayerUseC2SPacket(mc.player.displayItemInHand))
    ...
    val onTick = handler<GameTickEvent> {
        ...
        if (mc.screen != null || mc.interactionManager.gameMode == WorldSettings.GameMode.SPECTATOR
            || mc.interactionManager.gameMode == WorldSettings.GameMode.CREATIVE
        ) return@handler
