...
class WebTheme(val folder: File) : Theme {
    ...
    override val wallpapers: List<Wallpaper> = folder.resolve("wallpapers").listFiles()?.mapNotNull { file ->
            runCatching {
    ...
    override val textures: Map<String, Lazy<Identifier>> = folder.resolve("textures").listFiles { _, name ->
        ...
        lazy {
            ...
            mc.textureManager.registerTexture(
                identifier, NativeImageBackedTexture(NativeImage.read(file.inputStream()))
            )
