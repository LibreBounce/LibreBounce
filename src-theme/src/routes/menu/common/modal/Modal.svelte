...
<style lang="scss">
  ...
  .modal-wrapper {
    ...
    width: 100vw;
    height: 100vh;
    background-color: rgba(colors.$menu-base-color, 0.5);
    z-index: 99999;
  }

  .modal {
    background-color: rgba(colors.$menu-base-color, 0.7);
    min-width: 500px;
    position: fixed;
    left: 50%;
    top: 50%;
    ...
    border-radius: 5px;
    box-shadow: 0 0 10px rgba(colors.$menu-base-color, 0.5);
  }
  ...
  .button-modal-close {
    ...
    border: solid 2px colors.$menu-text-color;
    border-radius: 50%;
    cursor: pointer;
    top: 20px;
    right: 20px;
    ...
    &:hover {
      background-color: rgba(colors.$menu-text-color, 0.1);
    }
  }
